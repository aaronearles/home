...
# TODO Check for existing vm/template id
# TODO Secure password import
# Setup TEMPLATEID, VMID, VMNAME
# Vars for ssh keys/path, username, ipconfig, domain, disk size
# Bool for create-vm? more conditionals.
# Add support (or menu/variable) for other distros.
# Cleanup cloud image once complete?
...
# Generage cicustom vendor.yaml content
...
# Apply some tags
qm set 8000 --tags ubuntu-template,22.04,cloudinit

# Resize image disk to 8GB
qm resize 8000 scsi0 8G

qm template 8000

# Create (and start) a VM from template
# qm clone 8000 $VMID --name $VMNAME && qm start $VMID
